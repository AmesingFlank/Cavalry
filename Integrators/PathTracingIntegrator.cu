...
namespace PathTracing {
    ...
    void intersectScene( SceneHandle scene, SamplerObject sampler, TaskQueue<LightingTask> lightingQueue,TaskQueue<RayTask> thisRoundRayQueue, TaskQueue<RayTask> nextRoundRayQueue,int depth) {
        ...
        LightingTask lightingTask = { intersection,thisRay,multiplier,result,myTask.materialPDF,myTask.samplingState, myTask.sampledFromDeltaBSDF };
    ...
    void genNextRay(SceneHandle scene, SamplerObject sampler, TaskQueue<LightingTask> tasks, TaskQueue<RayTask> nextRoundRayQueue, int depth) {
        ...
        RayTask nextTask = { nextRay,multiplier,result,nextRayProbability, intersection.bsdf.isDelta(),myTask.samplingState };
    ...
    void genInitialRays(CameraSample* samples, int samplesCount, CameraObject camera, Spectrum* results, TaskQueue<RayTask> rayQueue,SamplerObject sampler,unsigned long long lastSampleIndex) {
        ...
        *result = make_float3(0, 0, 0);
        ...
        RayTask task = { ray,multiplier,result,1,true,samplingState };
        rayQueue.push(task);
    }
