...
unsigned int shiftMorton(unsigned int x){
    if (x == (1 << MORTEN_BITS_PER_DIMENSION)) --x;
...
void buildRadixTree(int leavesCount, BVHLeafNode* leaves, BVHInternalNode* internals){
    ...
    for(int power = 1;; ++power){
        unsigned int t = ceil( (float)length / ((float) (i<<power)) );
        if(t == 0){
            break;
        }
    ...
    internals[i].rightChildIsLeaf = max(i,j)== rightChild;
...
BVH BVH::build(Triangle* primitivesDevice, int primitivesCount,const AABB& sceneBounds){
    ...
    CHECK_CUDA_ERROR("fill leaf bounding boxes");
    ...
    CHECK_CUDA_ERROR("fill leaf morton");
    ...
    CHECK_CUDA_ERROR("build radix tree");

    computeBounds <<< numBlocksInternals, numThreadsInternals >>> (primitivesCount,leaves.data,internals.data);
    CHECK_CUDA_ERROR("compute bounds");


    mergeNodesArray <<< numBlocksInternals, numThreadsInternals >>> (primitivesCount,leaves.data,internals.data, bvh.nodes.gpu.data);
    CHECK_CUDA_ERROR("merge nodes array");    
